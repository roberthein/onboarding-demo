import Foundation

public extension FloatingPoint {

    /// Maps a value from one closed range to another. Useful for progress or interpolation.
    /// - Parameters:
    ///   - from: The source range (e.g. `0...1`).
    ///   - to: The target range (e.g. `0...100`).
    /// - Returns: The value linearly mapped to the target range, or `self` if ranges are invalid.
    func map(from: ClosedRange<Self>, to: ClosedRange<Self>) -> Self {
        guard from.lowerBound <= from.upperBound, to.lowerBound <= to.upperBound, from.upperBound - from.lowerBound != 0 else {
            return self
        }
        return ((self - from.lowerBound) / (from.upperBound - from.lowerBound)) * (to.upperBound - to.lowerBound) + to.lowerBound
    }
}
